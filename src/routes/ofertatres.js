...
router.post('/ofertatres/new-ofertatres',  async (req, res) => {
  const { name, title, image, imagedos, imagetres, description, price, oldprice, color, colorstock, talle, tallestock  } = req.body;
  const errors = [];
  if (!image) {
  ...
  if (errors.length > 0) {
    ...
    const newNote = new Ofertatres({ name, title, image, imagedos, imagetres, description, price, oldprice, color, colorstock, talle, tallestock  });
